...
@runtime_checkable
class AmazonSearchClient(Protocol):
...
class AmazonCollectionAdapter:
    ...
    def __init__(
        self,
        scraper: AmazonSearchClient | None = None,
        scraper_factory: Callable[[], AmazonSearchClient] | None = None,
    ) -> None:
    ...
    @staticmethod
    def _default_scraper_factory() -> AmazonSearchClient:
    ...
    def extract_product_id(url: str) -> str:
        ...
        return product_identity("Amazon", url)
    ...
    def collect(self, term: str, limit: int | None = None) -> tuple[dict[str, Any], ...]:
        ...
        try:
            ...
            if not isinstance(products, (list, tuple)):
                raise TypeError("Amazon retornou coleção inválida")
            return tuple(
            ...
            if isinstance(error, AmazonCollectionError):
                raise
            raise AmazonCollectionError(
