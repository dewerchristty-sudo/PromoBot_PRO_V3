...
from ..adapters import MercadoLivreCollectionError
...
class MercadoLivreKeywordCollector:
    SOURCE_TYPE = "keyword"
    STORE = "mercado livre"
    ...
    def collect(self, source: PromotionSource) -> CollectionResult:
        ...
        if source.store.strip().casefold() != self.STORE:
            raise ValueError("O coletor aceita somente Mercado Livre")

        started_at = datetime.now(timezone.utc)
        keyword = self._keyword(source)
        try:
            products = self.adapter.collect(keyword, source.limit)
        except MercadoLivreCollectionError as error:
