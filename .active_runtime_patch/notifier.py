...
class Notifier:
    ...
    def send_alerts(self, alerts, database=None, enforce_offer_quality=True):
    ...
    def send_manual_alerts(self, alerts, database=None):
    ...
    def offer_price_lines(self, item):
        ...
        current_text = (
            ...
            else Parser.format_brl(
                self.value(item, "preco"),
                include_symbol=True,
            )
        )
        lines = []
    ...
    def comparison_price(self, item):
        ...
        values = [self.price_number(candidate) for candidate in candidates]
    ...
    def format_money(self, value):
        ...
        return Parser.format_brl(value, include_symbol=True)
    ...
    def send_whatsapp_alerts(self, alerts):
        ...
        for index, item in enumerate(alerts[:10]):
            ...
            imagem = self.verified_whatsapp_image(item)
    ...
    def whatsapp_category(self, item):
        ...
        text = self.searchable_text(" ".join([
            ...
            str(self.value(item, "categoria_original", "") or ""),
            str(self.value(item, "breadcrumb", "") or ""),
        ]))
