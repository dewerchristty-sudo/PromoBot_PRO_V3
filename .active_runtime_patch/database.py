...
class Database:
    ...
    def migrar_tabelas(self):
        ...
        if "preco_antigo" not in colunas:
            self.cursor.execute(
                "ALTER TABLE produtos ADD COLUMN preco_antigo REAL DEFAULT 0"
            )
        ...
        if "categoria_original" not in colunas:
            self.cursor.execute(
                "ALTER TABLE produtos ADD COLUMN categoria_original TEXT DEFAULT ''"
            )
        ...
        try:
            ...
            with self.lock:
                ...
                ON CONFLICT(link) DO UPDATE SET
                    ...
                    preco_antigo = CASE
                        WHEN excluded.preco_antigo > excluded.preco_valor
                        THEN excluded.preco_antigo
                        ELSE produtos.preco_antigo
                    END,
