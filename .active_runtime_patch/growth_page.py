...
class GrowthPage(ctk.CTkFrame):
    ...
    def _offers_loaded(self, offers):
        self.offers_by_label = {
            f"{p['loja']} | {p['titulo'][:85]} | "
            f"{Parser.format_brl(p['preco'], True)}": p for p in offers
        }
        ...
        if kind == "convite":
            ...
            if kind == "social":
                text = (
                    ...
                    f"Oferta por {Parser.format_brl(price, True)}. "
                    "Preço sujeito a alteração pela loja.\n\n"
                ...
                text = (
                    f"OFERTA DO DIA\n\n{title}\n\n"
                    f"Por {Parser.format_brl(price, True)}\n\n"
...
"""
    ...
    def load_results(self):
        ...
        self.results_text.insert(
            ...
            f"Vendas: {sales}\n"
            f"Comissão: {Parser.format_brl(commission, True)}\n"
